...
export const DEFAULT_PROMPTS = {
   ...
   */
  TEST_DATA_GENERATOR: ` 
  -Generate ONLY a valid test data based on the selected field in the DOM.
 -Each field should have multiple test data
 -LIMIT the number of rows to be 8
 - Use realistic values for names, addresses, emails, phone numbers, and other fields.
 - use dropdown values only from provided DOM
 - Do NOT include explanations or code comments.
  ...
  Context:
    ...
    \`\`\`
     ...
     Example:
| Name          | Address                          | Pin Code | Mobile Number | From      | To         | Date of Journey | Booking for Women |
|---------------|----------------------------------|----------|---------------|-----------|------------|------------------|--------------------|
| jai           | M54LW, Salford                    | 2587412 | 9176765358    | thanjavur | Chennai    | 13 Sep, 2025     | Yes                |

   
  Persona:
    - Audience: Automation engineer needing reusable, realistic test data generators.

    Output Format:
    - Test data in a markdown table inside a \`\`\`markdown\`\`\` block.
  ...
  Instructions:
    ...
    - Use Playwright compatible imports.
    - Use meaningful method names.
    - Do NOT include explanations or test code.
